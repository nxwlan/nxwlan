...
void handle_auth(struct hostapd_data *hapd,
			const struct ieee80211_mgmt *mgmt, size_t len, int cli)
#else
static void handle_auth(struct hostapd_data *hapd,
				 const struct ieee80211_mgmt *mgmt, size_t len)
#endif /*STA_MOVING_ENABLED*/
{
	...
	case WLAN_AUTH_OPEN:
...
#if defined (IEEE80211_REQUIRE_AUTH_ACK) && ! defined (STA_MOVING_ENABLED)
		...
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
						  HOSTAPD_LEVEL_INFO,
						  "ACKless authentication enabled, (STA_MOVING_ENABLED)");
		sta->flags |= WLAN_STA_AUTH;
		wpa_auth_sm_event(sta->wpa_sm, WPA_AUTH);
 ...
 fail:
	...
	{
		printf("Auth is injected, we do not send an auth reply!\n");
	}
#else
	send_auth_reply(hapd, mgmt->sa, mgmt->bssid, auth_alg,
					auth_transaction + 1, resp, resp_ies, resp_ies_len);
			   ...
			   const u8 *ies, size_t ies_len, int reassoc)
{
	...
	int channel,p,q,s;
	const u8 *ie_sta_capa;
	size_t ie_len_sta_capa;
	u8 new_sta;
	u8 sa[6];
	struct sta_capa_entry *tmp_sta_capa_entry, *tmp_sta_capa_entry_two, *result_sta_capa_entry, *new_sta_capa_entry;
	new_sta=1;
	for(s=0;s<6;s++)
	{	
		sa[s]=sta->addr[s];
	...
	/*Find out if we already know this STA*/
					...
					struct sta_capa_entry, list)
	{
		...
		for(q=0; q<6; q++)
		{
			if(tmp_sta_capa_entry->mac[q] == sa[q])
			{
				p++;
			}
		}
		...
		{
			new_sta=0;
			result_sta_capa_entry = tmp_sta_capa_entry;
			break;
		}
	}
	if(new_sta==0)
	{
		...
		//printf("New data for capa db, STA: %x:%x:%x:%x:%x:%x via assoc req\n",
		//			result_sta_capa_entry->mac[0],result_sta_capa_entry->mac[1],
		//			result_sta_capa_entry->mac[2],result_sta_capa_entry->mac[3],
		//			result_sta_capa_entry->mac[4],result_sta_capa_entry->mac[5]);
		
		ie_len_sta_capa = ies_len;
		//Get memory for storing the IE data
		ie_sta_capa = os_malloc(ie_len_sta_capa);
		if (ie_sta_capa == NULL)
		{
		...
		//Copy IE data
		os_memcpy(ie_sta_capa, ies, ie_len_sta_capa);
		
		if (ieee802_11_parse_elems(ie_sta_capa, ie_len_sta_capa, &(result_sta_capa_entry->elems), 0) == ParseFailed) {
			wpa_printf(MSG_DEBUG, "Could not parse IE(s) from Assoc req for STA capa DB: " MACSTR,
			   MAC2STR(sa));
			return;
		}		
	}
	...
	{
		hapd->sta_capa_count++;
		//printf("New STA in capa DB (AssocReq), now: %d entries\n",hapd->sta_capa_count);
		new_sta_capa_entry = os_malloc(sizeof(struct sta_capa_entry));
		if (new_sta_capa_entry == NULL)
		{
			printf("Malloc returned -1\n");
			return -1;
		}
		//Add Mac to STA capa db
		for(s=0;s<6;s++)
		{	
			new_sta_capa_entry->mac[s]=sa[s];
		}
		//Detect 2.4 or 5 GHz support
		if(channel >= 36 && channel <=165) /*We assume it is 5GHz, as we only support 
		2.4 and 5 GHz, should be devided in more detail in future*/
		{
			new_sta_capa_entry->probe_fiveghz_rec=1;
			new_sta_capa_entry->probe_twoghz_rec=0;
		}
		else if(channel >=1 && channel <=14)
		{
			new_sta_capa_entry->probe_twoghz_rec=1;
			new_sta_capa_entry->probe_fiveghz_rec=0;
		}
		//Get IE(s) out of the Probe Request and copy it into memory
		//ie_sta_capa = ies;
		ie_len_sta_capa = ies_len;
		//Get memory for storing the IE data
		ie_sta_capa = os_malloc(ie_len_sta_capa);
		...
		if (ieee802_11_parse_elems(ie_sta_capa, ie_len_sta_capa, &(new_sta_capa_entry->elems), 0) == ParseFailed) {
			wpa_printf(MSG_DEBUG, "Could not parse IE(s) for STA capa DB: " MACSTR,
			   MAC2STR(sa));
			return;
		}
		dl_list_add(&(*hapd->sta_capa_list), &new_sta_capa_entry->list);
	}
...
#endif
{
 ...
 fail:
#ifdef STA_MOVING_ENABLED
	printf("Send assoc response?: %d ",cli);
	if(!cli) {
		printf("Yes\n");
		     ...
		     struct hostapd_frame_info *fi)
{
	...
	u8 q,p;
	u8 blackmac[6];
	struct blacklist_mac_entry *tmp_bl_mac, *tmp_bl_mac_two;
	...
	if(hapd->blacklist_mac != NULL)
	{
			...
			struct blacklist_mac_entry, list)
		{
			...
			if(p==6)
			{
				printf("Rejecting station\n");
				printf("Blacklisted MAC: %x:%x:%x:%x:%x:%x\n",
					tmp_bl_mac->mac[0],
					tmp_bl_mac->mac[1],
					tmp_bl_mac->mac[2],
					tmp_bl_mac->mac[3],
...
#ifdef STA_MOVING_ENABLED
		handle_assoc(hapd, mgmt, len, 0, 0);
#else
...
#ifdef STA_MOVING_ENABLED
		handle_assoc(hapd, mgmt, len, 1, 0);
#else
...
#ifdef STA_MOVING_ENABLED
void handle_assoc_cb(struct hostapd_data *hapd,
			    const struct ieee80211_mgmt *mgmt,
			    size_t len, int reassoc, int ok, int cli)
#else
...
#endif
{
	...
	wpa_hexdump(MSG_INFO, "  * ASSOC-CB: supported rates", sta->supported_rates,
				sta->supported_rates_len);
#ifdef STA_MOVING_ENABLED
	if(cli)
	{
		os_memcpy(sta->supported_rates, "\x01\x05\x24\xb0\x48\x60\x6c", 7);
		sta->supported_rates_len=7;
		sta->aid = 1;
		sta->capability=0x431;
		sta->listen_interval = 1;
		sta->flags = 0x3;
		sta->qosinfo = 0x0;
	}
#endif
...
#ifdef STA_MOVING_ENABLED
	//Set 802.1X port enabled (indicate succcessfull 802.1x autehtication)
	if(cli)
...
#ifdef STA_MOVING_ENABLED
		handle_assoc_cb(hapd, mgmt, len, 0, ok, 0);
#else
...
#ifdef STA_MOVING_ENABLED
		handle_assoc_cb(hapd, mgmt, len, 1, ok, 0);
#else
