...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	hapd->blacklisted_macs_list=os_malloc(sizeof(struct dl_list));
	dl_list_init(&(*hapd->blacklisted_macs_list));
#endif /*CLIENT_BLACKLISTING*/

#ifdef STA_CAPA_COLLECT
	hapd->sta_capa_list=os_malloc(sizeof(struct dl_list));
	dl_list_init(&(*hapd->sta_capa_list));
	hapd->sta_capa_count=0;
