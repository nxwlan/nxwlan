...
{
	void *buf;
	unsigned char *radiotap_ver;
	unsigned short *radiotap_start;
	unsigned short radiotap_len;
	struct mac80211_hwsim_data *nic;

	buf = skb->data;

	radiotap_ver = (buf + 0);
	radiotap_start = (buf + 2);
	radiotap_len = *radiotap_start;

	if(radiotap_len > 256 || *radiotap_ver != 0) {
		printk("exit becoause of to big radiotap");
		//Kernel receiving something not radiotap
		dev_kfree_skb(skb);
		return NETDEV_TX_OK;
	}
	...
	list_for_each_entry(nic, &hwsim_radios, list) {
		struct sk_buff *nskb;
		if(nic->idle || !nic->started || !nic->channel) continue;

		nskb = skb_copy(skb, GFP_ATOMIC);
		if(nskb == NULL) continue;
				      ...
				      struct ieee80211_channel *chan)
{
	...
	//hdr->rt_dbm_antsignal = -60; // CloudMAC
