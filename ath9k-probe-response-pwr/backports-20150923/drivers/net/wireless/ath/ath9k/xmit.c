...
static u8 ath_overwrite_rate_txpower(struct ath_softc *sc, struct ath_buf *bf,
 			       u8 rateidx, bool is_40, bool is_cck, u8 oldPower)
 {
    ...
    packet_type = get_hw_packet_type(skb);
    if( packet_type == ATH9K_PKT_TYPE_PROBE_RESP)
    {
        if(info->flags & IEEE80211_TX_CTL_INJECTED)
        {
            printk("Sent a injected Probe Response (mark=10001)\n");
            mymark = 10001;
        }
        else
        {
            printk("Sent a NON injected Probe Response (mark=10002)\n");
            mymark = 10002;
        }
    }
    else
    {
 	...
 	list_for_each_entry(tmp, &(per_flow_txpower_list->list), list) 
 	{
 		...
 		{
 			max_power = tmp->power;
            printk("Entry for mark %d found, using txpower: %d, old_power: %d\n",mymark,max_power, oldPower);
 			break;
 		}
     }
			     ...
			     struct ath_tx_info *info, int len, bool rts)
{
	...
	rates = bf->rates;
	hdr = (struct ieee80211_hdr *)skb->data;
    //printk("ath_buf_set_rate called\n");
    packet_type = get_hw_packet_type(skb);
    //printk("Packet Type: %d, Probe Response== %d\n",packet_type, ATH9K_PKT_TYPE_PROBE_RESP);
			       ...
			       struct ath_atx_tid *tid, struct sk_buff *skb)
{
	...
	struct list_head bf_head;
	struct ath_buf *bf = fi->bf;
    //ath_overwrite_rate_txpower(sc, bf, rix, is_40, false, fi->tx_power);
    //printk("ath_tx_send_normal() with pwr:%d\n", fi->tx_power);
	INIT_LIST_HEAD(&bf_head);
	list_add_tail(&bf->list, &bf_head);
	bf->bf_state.bf_type = 0;
