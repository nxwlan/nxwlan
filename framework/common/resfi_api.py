...
class ResFiNorthBoundAPI(object):
    ...
    def getMacAddrAssociatedClients(self):
        """
            Get MAC addresses of associated client stations.
        """
        return
...
class ResFiSouthBoundAPI(object):
    ...
    def getSSID(self):
        """
        Getting the SSID used by AP.
        :param data,    returns currently used SSID
        """
        return
...
class AbstractResFiApp(ResFiNorthBoundAPI, Thread):
    ...
    @abc.abstractmethod
    def probeReqRec_cb(self, nodeID):
        return
    ...
    def getTxBitrateStation(self, mac_addr):
        return self.agent.getTxBitrateStation(mac_addr)

    def getMACTxBytesStation(self, mac_addr):
        return self.agent.getMACTxBytesStation(mac_addr)

    def getMacAddrAssociatedClients(self):
        return self.agent.getMacAddrAssociatedClients()

    def startVAP(self, ssid, neighbor_ip):
        return self.agent.startVAP(ssid, neighbor_ip)

    def getSSID(self):
        return self.agent.getSSID()

    def getHostname(self):
        return self.agent.getResFiCredentials(1)