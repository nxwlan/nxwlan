...
class LinuxConnector(AbstractConnector):
    ...
    def getNetworkLoad(self):
        tmp = self.getInfoOfAssociatedSTAs()
        if tmp is None:
            return 0
        num_stas = len(tmp)
        return num_stas
    ...
    """
        Get the PHY TX bitrate (DL) for a given client station.
    """
    ...
    """
        Get the numer of transmitted bytes (DL) towards the client station.
    """
    def getMACTxBytesStation(self, mac_addr):
        ...
        if tmp is None:
            return 0
        return float(tmp[mac_addr]['tx bytes'][0])

    """
        Get MAC address of all associated stations.
    """
    ...
    def getInfoOfAssociatedSTAs(self):
        try:
            ...
            self.log.warn("An error occurred: %s" % e)
            return None


    def startVAP(self, ssid, neighbor_ip):
        self.log.debug('Linux:: startVAP()')
        print "FAKE!! NOT IMPLEMENTED!!!: VAP with SSID "+str(ssid)+" started"
        print "Connection to "+str(neighbor_ip)+" successfully established"
...
class MininetConnector(AbstractConnector):
    ...
    def startVAP(self, ssid, neighbor_ip):
        self.log.debug('Mininet:: startVAP()')
        print "FAKE!! NOT IMPLEMENTED!!!: VAP with SSID "+str(ssid)+" started"
        print "FAKE!! NOT IMPLEMENTED!!!: Connection to "+str(neighbor_ip)+" successfully established"
